...
impl WalletManager {
    ...
    pub fn to_keyfile<P: AsRef<Path>>(&self, dir: P, password: &str, name: Option<String>) -> Result<(), WalletError> {
        ...
        let name_val = name.unwrap_or_else(|| format!("UTC--{}--{:x}", 
        ...
        let content = fs::read_to_string(&keyfile_path)
            .map_err(|e| WalletError::KeyfileWrite(format!("failed to post-process keyfile: {}", e)))?;
        ...
        fs::write(&keyfile_path, updated)
            .map_err(|e| WalletError::KeyfileWrite(format!("failed to write finalized keyfile: {}", e)))?;
    ...
    pub async fn sign_typed_data(&self, typed_data: TypedData) -> Result<Signature, WalletError> {
        self.wallet.sign_typed_data(&typed_data).await
            .map_err(|e| WalletError::Operation(sanitize_error(&e)))
    }

    /// Signs a transaction request and returns the signature.
    #[instrument(skip(self, tx))]
    pub async fn sign_transaction(&self, tx: &TransactionRequest) -> Result<Signature, WalletError> {
        debug!("Signing transaction");
        tx.validate()?;

        let request: TypedTransaction = tx.to_ethers_request().into();
        self.wallet.sign_transaction(&request).await
            .map_err(|e| WalletError::Operation(sanitize_error(&e)))
    }

    /// Signs a transaction and returns the RLP-encoded signed bytes.
    pub async fn sign_transaction_bytes(&self, tx: &TransactionRequest) -> Result<Vec<u8>, WalletError> {
        tx.validate()?;
...
impl fmt::Display for WalletManager {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}({}={})", stringify!(WalletManager), ADDRESS_KEY, self.address())
    }
}
